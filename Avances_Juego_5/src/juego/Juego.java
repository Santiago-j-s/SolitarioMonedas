...
/**
 * La clase Tablero permite instanciar un objeto tablero
...
public class Juego {
	...
	/**
	 * Constructor. Inicializa un tablero de juego. La cantidad de monedas 
	 * 		inicial se deduce de la cantidad de casillas disponibles que no son nulas o
	 * 		vacias.
	 * 
	 * @param cantFilas - cantidad de filas que contendrá el tablero
	 * @param cantColumnas - cantidad de filas que contendrá el tablero
	 * @param cantNulas - cantidad de casillas nulas que contendrá el tablero
	 * @param cantVacias - cantidad de casillas vacias que contendrá el tablero
	 * 
	 */
	...
	/**
	 * Constructor defecto. 5 filas, 5 columnas, 
	 * 						9 casillas nulas, 2 casillas vacias
	 */
	public Juego() {
		this(5, 5, 9, 2);
	}
	...
	/**
	 * @param cantSaltos the cantSaltos to set
	 */
	 ...
	 * 		y si la moneda puede saltar en la dirección indicada.
	...
	public boolean puedeSaltar(int fila, int columna, Direccion direccion) {
		...
		switch(direccion) {
		case Arriba : return tablero.esMoneda(fila, columna) 
				&& tablero.esMoneda(fila - 1, columna) 
				&& tablero.esVacia(fila - 2, columna);
		case Abajo : return tablero.esMoneda(fila, columna) 
				&& tablero.esMoneda(fila + 1, columna) 
				&& tablero.esVacia(fila + 2, columna);
		case Derecha : return tablero.esMoneda(fila, columna) 
				&& tablero.esMoneda(fila, columna + 1) 
				&& tablero.esVacia(fila, columna + 2);
		case Izquierda : return tablero.esMoneda(fila, columna) 
				&& tablero.esMoneda(fila, columna - 1) 
				&& tablero.esVacia(fila, columna - 2);
	 ...
	 * 		dicha moneda puede saltar en cualquier dirección. 
	 * 		false en caso contrario
	 */
	...
	/**
	 * Ejecuta el salto de una moneda a una casilla
	 * situada en los parametros fila,columna con 
	 * el movimiento en direccion del parametro
	 * @param fila
	 * @param columna
	 * @param cadenaDireccion
	 */
	public void saltar(int fila, int columna, String cadenaDireccion) {
		
		Direccion direccion = Direccion.Arriba;
		
		try {
			direccion = Direccion.valueOf(
					cadenaDireccion.substring(0, 1).toUpperCase() + 
					cadenaDireccion.substring(1).toLowerCase()
				);
		...
		if (puedeSaltar(fila, columna, direccion)) {
			...
			switch(direccion) {
			case Arriba : tablero.setCasilla(fila, columna, Casilla.Vacia);
						  tablero.setCasilla(fila - 1, columna, Casilla.Vacia);
						  tablero.setCasilla(fila - 2, columna, Casilla.Moneda);
						  break;
						  
			case Abajo : tablero.setCasilla(fila, columna, Casilla.Vacia);
						 tablero.setCasilla(fila + 1, columna, Casilla.Vacia);
						 tablero.setCasilla(fila + 2, columna, Casilla.Moneda);
			             break;
			             
			case Derecha : tablero.setCasilla(fila, columna, Casilla.Vacia);
						   tablero.setCasilla(fila, columna + 1, Casilla.Vacia);
						   tablero.setCasilla(fila, columna + 2, Casilla.Moneda);
						   break;
 			 			   
			case Izquierda : tablero.setCasilla(fila, columna, Casilla.Vacia);
							 tablero.setCasilla(fila, columna - 1, Casilla.Vacia);
							 tablero.setCasilla(fila, columna - 2, Casilla.Moneda);
 			                 break;
			}
	...
	public boolean victoria() {
		...
		if(tablero.getCantMonedas() == 1) return true;
		else return false;
	}
	
	/**
	 * @return true si ninguna moneda puede saltar en ninguna dirección.
	 * false si al menos una moneda puede saltar en alguna dirección.
	 */
